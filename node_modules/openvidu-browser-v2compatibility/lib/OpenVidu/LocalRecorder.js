...
const LocalRecorderState_1 = require("../OpenViduInternal/Enums/LocalRecorderState");
...
class LocalRecorder {
    ...
    record(options) {
        return new Promise((resolve, reject) => {
            try {
                ...
                if (this.state !== LocalRecorderState_1.LocalRecorderState.READY) {
    ...
    stop() {
        return new Promise((resolve, reject) => {
            try {
                if (this.state === LocalRecorderState_1.LocalRecorderState.READY || this.state === LocalRecorderState_1.LocalRecorderState.FINISHED) {
    ...
    pause() {
        return new Promise((resolve, reject) => {
            try {
                if (this.state !== LocalRecorderState_1.LocalRecorderState.RECORDING) {
    ...
    resume() {
        return new Promise((resolve, reject) => {
            try {
                if (this.state !== LocalRecorderState_1.LocalRecorderState.PAUSED) {
    ...
    download() {
        if (this.state !== LocalRecorderState_1.LocalRecorderState.FINISHED) {
            ...
            a.href = url;
            a.download = this.id + '.' + mime_1.default.getExtension(this.blob.type);
            a.click();
     ...
     */
    uploadAsBinary(endpoint, headers) {
        return new Promise((resolve, reject) => {
            if (this.state !== LocalRecorderState_1.LocalRecorderState.FINISHED) {
     ...
     * @returns A Promise (to which you can optionally subscribe to) that is resolved with the `http.responseText` from server if the operation was successful and rejected with the failed `http.status` if not:
     */
    uploadAsMultipartfile(endpoint, headers) {
        return new Promise((resolve, reject) => {
            if (this.state !== LocalRecorderState_1.LocalRecorderState.FINISHED) {
                ...
                sendable.append('file', this.blob, this.id + '.' + mime_1.default.getExtension(this.blob.type));
                http.onreadystatechange = () => {
                    if (http.readyState === 4) {
                        if (http.status.toString().charAt(0) === '2') {
                            // Success response from server (HTTP status standard: 2XX is success)
